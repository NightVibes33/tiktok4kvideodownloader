...
struct NativeSlideshowDownloaderView: View {
    @StateObject private var model = NativeSlideshowViewModel()
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(spacing: 18) {
                    urlInput
                    if let success = model.successMessage { SuccessBanner(message: success) }
    ...
    private var urlInput: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Button(action: { Task { await model.extract() } }) {
                ...
                else { Text("Extract Slideshow").frame(maxWidth: .infinity) }
            }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                Button("Download All Photos") {
                    Task { for imageURL in model.images { await model.savePhoto(imageURL) } }
                }
                .buttonStyle(PrimaryButtonStyle())
