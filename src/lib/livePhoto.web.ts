...
export async function downloadImageInBrowser(imageUrl: string, baseFilename: string): Promise<void> {
...
export async function shareOrDownloadImageOnIOS(imageUrl: string, baseFilename: string): Promise<void> {
  const file = await fetchImageFile(imageUrl, baseFilename);
  ...
  await downloadImageInBrowser(imageUrl, baseFilename);
}
...
export async function downloadLivePhoto(
  ...
  if (!assets.videoUrl) {
    throw new Error("Missing paired video for Live Photo save.");
  }
